...
const DOMAIN = "http://15.164.251.60:8080";
...
export const api = {
	...
	orders: {
		// 주문목록 조회
		OfindAll: () => request.get("/orders"),
		// 주문 조회
		OfindOne: (id) => request.get(`/orders/${id}`),
		// 주문하기
		Ocreate: (menus_id, quantity, request_detail) => request.post(`/orders`, {
			menus_id,
			quantity,
			request_detail
		}),
		// 주문 내역 수정하기
		Oupdate: (id, menus_id, quantity, request_detail) => request.patch(`/orders/${id}`, {
			menus_id: menus_id,
			quantity,
			request_detail
		}),
		// 주문 내역 삭제하기 
		Odelete: (id) => request.delete(`/orders/${id}`)
	}
}
